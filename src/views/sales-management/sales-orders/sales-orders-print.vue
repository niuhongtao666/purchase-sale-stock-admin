          ...
          <div class="item mb-10 printZone table-unset">
            <table class="table ">
              ...
              <tr v-if="page==pages">
                ...
                <td colspan='4' style="width: 50%">应收金额合计: {{totalReceivePrice}}</td>
              </tr>
              <tr>
                <td colspan="8">
...
<script>
  export default {
    ...
    computed: {
      ...
      allData(){
        this.printData.saleOrderDetialDTOS.forEach((item,index)=>{
          if(item.favorablePrice<0){
            item['unitPrice']=this.AppUtils.accDiv(Number((item.orderNum*item.unitPrice-item.favorablePrice).toFixed(2)),item.orderNum);
            item.favorablePrice=0;
          }
        });
        return this.printData.saleOrderDetialDTOS;
      },
      pages() {
        ...
        *
        ...
        var pages_ = Math.ceil(
          this.printData.saleOrderDetialDTOS.length / this.onePageRow
        ); // 向上取整数
