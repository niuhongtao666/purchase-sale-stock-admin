        ...
        <div class="select-wrap">
          <div class="group">
            ...
            <div class="group ml0">
              <span class="left"></span>
              <div class="right">
                <input type="text" placeholder="请输入出库单号" v-model.trim="model.checkOutputOrderNo" style="width: 121px">
              </div>
            </div>
...
<script>
  export default {
    ...
    data(){
      return{
        model:{
          ...
          checkOutputStoreId:null,//出库门店id
          checkOutputOrderNo:null,//盘亏出库编号
          pageSize:20,//	页数
          pageNo:1,//页码
        },
        data:{
    ...
    mounted(){
      ...
      // this.buttonInfo=this.AppUtils.returnPermissions(154);
    },
    computed: {
      buttonInfo() {
        return this.AppUtils.returnPermissions(154);
      },
    },
    methods:{
      getSummaries(param) {
      ...
      exportData(){
        ...
        delete obj['pageNo'];
        delete obj['pageSize'];
        this.$axios.post('/admin/stock/stockCheckOutputOrderInfo/export',obj,{responseType:'arraybuffer'}).then(res => {
          var myDate = new Date();
          var time = myDate.toLocaleDateString().split('/').join('-');
          let fileName = '盘亏出库单'+time+'.xls';
      ...
      fetchData(args){
        ...
        this.$axios.post('/admin/stock/stockCheckOutputOrderInfo/list',this.model).then(res=>{
