<template>
  <div>
        ...
        <div class="rowItem h-49 borderBottom">
          <div class="left h-49">
            ...
            <div class="rightZone resetInput">
              {{data.costAdjustmentNo}}
            </div>
          </div>
          <div class="left h-49">
            <div class="leftZone">调整类型</div>
            <div class="rightZone resetInput">
              {{data.adjustmentType==1?'入库成本调整':''}}
            </div>
          </div>
          <div class="left h-49">
      ...
      <div class="table_wrap">
        <div class="table_layout" style="padding:25px;">
          <div class="table_scroll add">
            <el-table
              :data="data.stockCostAdjustmentDetailDTOS"
              max-height="582"
              border
              tooltip-effect="dark"
              style="width: 100%"
    ...
    <print-box v-if="printShow" @hidePrintMask="hidePrintMask">
      <template >
        <costAdjustPrint :data="data"></costAdjustPrint>
      </template>
    </print-box>
  </div>
</template>
...
<script>
  ...
  import costAdjustPrint from './costAdjustPrint'
  export default {
    name:'cost-adjust-detail',
    components:{
      printBox,
      costAdjustPrint
    },
    ...
    methods:{
      ...
      fetchData() {
        this.$axios.get('/admin/stock/stockCostAdjustment/detail/'+this.adjustId).then(res=>{
