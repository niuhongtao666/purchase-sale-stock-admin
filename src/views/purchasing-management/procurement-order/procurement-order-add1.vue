...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      addParts(data) {
        //console.dir(data)
        data['unitPrice']=data['lastPrice'];//model.purchaseOrderInfoDTO.taxRate
        if(data['unitPrice']){
        ...
        if (index > -1) {
          return this.AppUtils.confirm('当前配件已经添加,是否确定继续添加?', () => {
            ...
            ////console.log(this.model.purchaseOrderDetailDTOS[index]['unitPrice']);
            ////console.log(this.model.purchaseOrderDetailDTOS[index]['orderNum'])
      ...
      getSummaries(param) {
          ...
          if(!_this.appToOrderId){
            ...
            if (index === 4) {
              ...
              this.model.purchaseOrderInfoDTO.orderTotalPrice=this.AppUtils.decimal2(total);
