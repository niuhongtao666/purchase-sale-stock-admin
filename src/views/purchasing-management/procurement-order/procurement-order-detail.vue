<template>
  <div>
    <div class="applicationInfo">
      ...
      <div class="row">
        ...
        <div class="rowItem h-49 borderBottom">
          ...
          <div class="left h-49">
            <div class="leftZone" style="font-size: 12px!important;">采购总价(含税)</div>
            <div class="rightZone">
              {{model.purchaseOrderInfoDTO.orderTotalPrice | decimalTofixed2}}
            </div>
          </div>
          <div class="left h-49">
        ...
        <div class="rowItem h-49 borderBottom">
          ...
          <div class="left h-49">
            <div class="leftZone" style="font-size: 12px!important;">入库总价(含税)</div>
            <div class="rightZone">
              {{model.purchaseOrderInfoDTO.inputTotalPrice | decimalTofixed2}}
            </div>
          </div>
          <div class="left h-49">
            <div class="leftZone font-size12" style="white-space: nowrap;">入库总价(不含税)</div>
            <div class="rightZone">
              {{model.purchaseOrderInfoDTO.inputNoTaxPrice | decimalTofixed2}}
            </div>
          </div>
          <div class="right h-49">
        ...
        <div class="table_layout" style="padding:0px 25px;">
          <div class="table_scroll add resetTd">
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <input type="number" v-model.number="scope.row.orderNum" class="in" min="1" @input="getTotal(scope.row.goodsId,scope.row.orderNum,scope.row.unitPrice)" v-if="model.purchaseOrderInfoDTO.orderState==100&&!model.purchaseOrderInfoDTO.purchaseApplyNo">
              ...
              >
                <template slot-scope="scope">
                  <input type="number"  v-model="scope.row.unitPrice" class="inputWidth" min="0" step='0.1' @input="getTotal(scope.row.goodsId,scope.row.orderNum,scope.row.unitPrice)" v-if="model.purchaseOrderInfoDTO.orderState==100">
              ...
              >
                <template slot-scope="scope">
                  ...
                  <input type="number"  v-model="scope.row.totalPrice" class="inputWidth" min="0" step='0.1' @input="getUnitPrice(scope.row.goodsId,scope.row.totalPrice)" v-if="model.purchaseOrderInfoDTO.orderState==100">
              ...
              >
                <template slot-scope="scope">
                  <el-select placeholder="请选择" class="el_box" v-model.trim="scope.row.locationName" @change="locationChange(scope.row)" @visible-change="fetchLocationInfoByWaveId(scope.row.wavehouseId,$event)" v-if="model.purchaseOrderInfoDTO.orderState==100" v-el-select-loadmore="loadmore" @blur="reBack" filterable remote :remote-method="searchLocation">
                    <el-option
    ...
    <importWare :importWareShow="importWareShow" @closeImportWare="closeImportWare" :importWareList="importWareList" @clearImportWareList="clearImportWareList" @setImportWareList="setImportWareList" @fetchData="fetchData" :purchaseOrderId="purchaseOrderId"  @selectAll="selectAll" :waveList="waveList" :rate="rate"></importWare>
    ...
    methods:{
      ...
      handleSelectionChange(val) {
        ...
        this.importWareList.forEach((val,index)=>{
          ...
          val['noTaxUnitPrice']=_this.AppUtils.decimal4(_this.AppUtils.accDiv(parseFloat(val['unitPrice']),(1+this.rate/100)));
      ...
      statusUpdate(arg){
        ...
        if(arg=='submitPurchase'){
          var arr=this.model.purchaseOrderDetailDTOS.filter(item=>Math.floor(item.orderNum)!=item.orderNum);
          if(arr.length){
            var infoArr=[];
            arr.forEach((item)=>{
      ...
      /*getTotal(id,orderNum,unitPrice) {
        ...
        var totalPrice=this.AppUtils.mul(parseFloat(orderNum),unitPrice);
      ...
      fetchData() {
        ...
        this.$axios.post('/admin/purchase/purchaseOrder/detail/'+this.purchaseOrderId).then(res=>{
          ...
          if(data) {
            ...
            this.rate=this.model.purchaseOrderInfoDTO.taxRate;
            // alert(this.rate)
            this.getWaveNew();
