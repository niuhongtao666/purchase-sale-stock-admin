        ...
        <div class="select-wrap">
          <div class="group">
            ...
            <div class="group type">
              <span class="left">出库类型</span>
              <div class="right">
                <el-select placeholder="请选择" class="el_box" v-model.trim="model.outputStockTypes" multiple collapse-tags>
                  <el-option
                    v-for="item in outgoingTypesList"
                    :key="item.id"
                    :label="item.name"
                    :value="item.id">
                  </el-option>
            ...
            <div class="group">
              <span class="left">出库状态</span>
              <div class="right">
                <el-select placeholder="请选择" class="el_box" v-model.trim="model.outputStockStatus">
                  <el-option
                    v-for="item in outgoingStatesList"
                    :key="item.id"
                    :label="item.name"
                    :value="item.id">
                  </el-option>
            ...
            <div class="group">
              <span class="left">结算状态</span>
              <div class="right">
                <el-select placeholder="请选择" class="el_box" v-model.trim="model.payStatus">
                  <el-option
        ...
        <div class="select-wrap">
          <div class="group">
            <div class="group ml0">
              ...
              <div class="right">
                <input type="text" placeholder="请输入客户名称" v-model.trim="model.customerName">
              </div>
            </div>
        ...
        <div class="select-wrap">
          <div class="group">
            ...
            <div class="group">
              <span class="left"></span>
              <div class="right">
                <input type="text" placeholder="请输入车牌号" v-model.trim="model.numberPlates">
              </div>
            </div>
          </div>
        </div>
        <div class="select-wrap">
          <div class="group">
            <div class="group">
              <span class="left"></span>
              <div class="right">
                <input type="text" placeholder="请输入业务人员" v-model.trim="model.salesmanName">
              </div>
            </div>
            <div class="group ml10">
      ...
      <div class="table_layout">
        <div class="table_scroll ">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                {{scope.row.outputUnitPrice | decimalTofixed2}}
              </template>
            </el-table-column>
            <el-table-column
              label="销售总价"
            ...
            >
              <template slot-scope="scope">
                {{scope.row.outputTotalPrice | decimalTofixed2}}
              </template>
            </el-table-column>
            <el-table-column
              prop="outputNoTaxUnitPrice"
              label="销售单价(不含税)"
              min-width="150"
              key="16"
              show-overflow-tooltip
            >
              <template slot-scope="scope">
                {{scope.row.outputNoTaxUnitPrice}}
              </template>
            </el-table-column>
            <el-table-column
              label="销售总价(不含税)"
            ...
            >
              <template slot-scope="scope">
                {{scope.row.outputNoTaxTotalPrice}}
              </template>
            </el-table-column>
            <el-table-column
              label="成本单价(不含税)"
            ...
            >
              <template slot-scope="scope">
                {{scope.row.costTotalPrice | decimalTofixed4}}
              </template>
            </el-table-column>
            <el-table-column
...
<script>
  export default {
    name:'parts-outgoing-data',
    data(){
      return{
        model:{
          startTime:null,//开始时间
          endTime:null,//结束时间
          outputStockTypes:[],//出库类型
          outputStockStatus:null,//出库状态 （1：出库；2：出库退货）
          payStatus:null,//结算状态
    ...
    mounted(){
      ...
      //获取一级分类
      this.getFirstClass();
      // this.buttonInfo=this.AppUtils.returnPermissions(22);
      var today=new Date();
    ...
    computed: {
      buttonInfo() {
        return this.AppUtils.returnPermissions(22);
      },
    },
    methods:{
      clearWave(arg){
        ...
        columns.forEach((column, index) => {
          if(_this.positionChange==0){
            if (index === 0) {
              ...
              sums[index] = this.AppUtils.decimal2(this.sumData.sumNoTaxSaleTotal);
            }else if(index === 16){
      ...
      exportData(){
        ...
        this.$axios.post('/admin/report/outputReport/export',this.model,{responseType:'arraybuffer'}).then(res => {
          var myDate = new Date();
          var time = myDate.toLocaleDateString().split('/').join('-');
          let fileName = '配件出库报表'+time+'.xls';
      ...
      fetchData(args){
        ...
        this.$axios.post('/admin/report/outputReport',this.model).then(res=>{
